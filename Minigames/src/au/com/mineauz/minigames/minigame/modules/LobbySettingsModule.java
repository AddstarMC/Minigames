...
public class LobbySettingsModule extends MinigameModule {
	...
	public Menu createSettingsMenu() {
		...
		menu.addItem(teleportOnPlayerWait.getMenuItem("Teleport After Player Wait", "Should players be teleported;after player wait time?", Material.ENDER_PEARL));
		menu.addItem(teleportOnStart.getMenuItem("Teleport on Start", "Should players teleport;to the start position;after lobby?", Material.ENDER_PEARL));
		menu.addItem(playerWaitTime.getMenuItem("Waiting for Players Time", "The time in seconds;the game will wait for;more players to join.;A value of 0 will use;the config setting", Material.WATCH));
